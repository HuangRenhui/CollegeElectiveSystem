...
本项目已完成**框架搭建 + 基础设施 + 大部分业务模块**。其中**选课模块**与**成绩模块**的
Service 实现类尚未编写，接口契约、数据模型、前端页面与基础设施均已就绪，待开发者补充实现。
...
| **选课模块** | ⏳ **待实现** | 浏览课程可用；选课、退课、时间冲突检测待实现 |
| **成绩模块** | ⏳ **待实现** | 成绩录入、发布、成绩单待实现 |
...
| 角色 | 主要能力 |
| --- | --- |
| **学生 STUDENT** | 浏览可选课程、选课 / 退课、查看个人课表、查询成绩单与绩点 |
| **教师 TEACHER** | 查看授课课程、查看学生名单、录入 / 发布 / 撤回成绩、查看教学课表 |
| **管理员 ADMIN** | 课程与排课管理、学生管理、教师管理、院系 / 专业 / 教室维护、学期与选课窗口设置、公告发布、操作日志审计、选课开关与缓存预热 |
...
- **逻辑删除**：除 `sys_log` 外所有表使用 `deleted` 字段（0 未删除 / 1 已删除），由 MyBatis-Plus 自动处理。
...
| Redis Key | 类型 | 说明 |
| --- | --- | --- |
| `elective:course:capacity:{courseId}` | String | 课程剩余容量 |
| `elective:course:selected:{courseId}` | Set | 已选学生 ID 集合 |
...
- [**待实现功能**](./docs/待实现功能.md) —— 选课与成绩模块实现指引
- [架构设计](./docs/架构设计.md)
- [数据库设计](./docs/数据库设计.md)
- [接口文档](./docs/接口文档.md)
- [部署运维](./docs/部署运维.md)
